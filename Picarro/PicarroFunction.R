...
readCRDS <- function(Folder,From=NULL,To=NULL,tz='ETC/GMT-1',rm=TRUE,ibts=FALSE,mult=FALSE){
# browser()	
	...
	out <- lapply(Picarro, function(CRDS_name){
			...
			# browser()
				...
				dt[, PICARRO := c(JF='GHG_Picarro',NO='Backpack_Picarro',AH='Ammonia_Picarro',CF='Isotope_Picarro')[substr(CRDS_name,1,2)]] # name of the Picarro
	...
	out <- setNames(out, paste(Picarro, c(JF='GHG_Picarro',NO='Backpack_Picarro',AH='Ammonia_Picarro',CF='Isotope_Picarro')[substr(Picarro,1,2)],sep='/'))
	if(length(out) == 1){
		out <- out[[1]] 
	} 
	return(out)
