...
readHOBO <- function(Folder, From = NULL, To = NULL, Device = NULL, latest = FALSE, cut = TRUE){
	...
	if(is.null(Device)) {
		dt_devices <- dt_files[order(date_time)]
	}	else {
		dt_devices <- dt_files[Dev %in% Device][order(date_time)]
		}
...
# browser()
	...
	HOBO_ls <- lapply(dt_devices[, unique(Dev)], function(k) {
		tryCatch({
			...
			if(latest) {
				dt_sub2 <- tail(dt_sub, n = 1)
			} else {
				dt_sub2 <- dt_sub
				}
			# select rough time range
			if(is.null(From)) {
				y_from <- fread(dt_sub[1, file_path], sep = ',', fill = TRUE)
				setnames(y_from, old = names(y_from)[2], new = 'Date')
				y_from[, Date := as.POSIXct(Date, '%m/%d/%Y %H:%M:%S', tz = 'CET')] # convert times
				FROM <- y_from[!is.na(Date), Date][1]
			} else {
				FROM <- convert_date(From)
				} # define start time
			if(is.null(To)) {
				TO <- tail(dt_sub2[, date_time], n = 1)
			} else {
				TO <- convert_date(To)
				} # define end time
			if(is.na(FROM) | is.na(TO)) {
				stop('Please enter a valid start and end time')
			} # error/warning message
			if(!is.null(From) & !is.null(To)) {
				if(From > To) {
					stop('Your chosen "From" time is after the chosen "To" time.')
				}
			} # error message
			if(FROM > tail(dt_sub2[, date_time], n = 1)) {
				stop('Your chosen "From" time is after the available data.')
			} # error message
			if(FROM > TO) {
			...
			device_ls <- lapply(files_select, function(x) {
				...
				if(any(grep('Dew', names(out)))) {
					...
					setnames(out, new = c('Device', 'Date', 'Temp', gsub(' ', '_', names(out[, 4:ncol(out)]))))
				}
...
convert_date <- function(x, tz = NULL, formate = NULL) {
  formate <- c("Y", "%d.%m.%Y", "%d.%m.%Y %H:%M", "%d.%m.%Y %H:%M:%S", "Ymd", "YmdHM", "YmdHMS")
  date_time <- parse_date_time(x, orders = formate, tz = tz)
  return(date_time)
}
